        ...
        if (audio.paused) {
            ...
            for (var i = 0; i < audios.length; i++) {
                if (i != index && !audios[i].paused) {
                    audios[i].pause();
                    document.getElementById('audioPlayer' + i).src = './img/play.png';
                }
            }
...
function audioEnded(index) {
    ...
    document.getElementById('audioPlayer' + index).src = './img/play.png';

    // 自动播放下一个音频
    var audios = document.getElementsByTagName('audio');
    var nextIndex = (index + 1) >= audios.length ? 0 : index + 1;
    audios[nextIndex].play();
    document.getElementById('audioPlayer' + nextIndex).src = './img/pause.png';
}
